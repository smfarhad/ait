            ...
            <div class="col-xs-12">
                <div class="box box-success">
                    ...
                    <div class="box-body">
                        <!-- /.box-header -->
                        <!-- form start -->
                        {{ Form::open(array('url' => 'admin/users/settings', 'class'=>'form-horizontal')) }}
                        <div class="box-body">
                                ...
                                <div class="col-sm-3">
                                    <select name="user" class="form-control" required>
                                        ...
                                        @if(count($data['users'])>0)
                                            ...
                                            <option value="{{$row->id}}" @if($row->id == $data['user']){{'selected'}} @endif >{{$row->name}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                                ...
                                @foreach($data['permission'] as $row)
                                    <div class="form-group col-md-12">
                                        <div class="form-group col-sm-3">
                                            ...
                                            <input type="hidden" name="module[]" class="form-control" id="module" value="{{$row->module_id}}">
                                        </div>
                                        <label style="heigh:50px; margin-left:50px;" for="read" class=" control-label">Read</label>
                                        <label><input name="read[]" type="checkbox" value="{{$row->module_id}}" class="minimal" @if($row->read_access == 1){{'checked'}} @endif></label>

                                        <label style="heigh:50px; margin-left:40px;" for="write" value="1" class=" control-label">Write</label>
                                        <label><input name="write[]" type="checkbox" value="{{$row->module_id}}" class="minimal" @if($row->write_access == 1){{'checked'}} @endif></label>
                                        
                                        <label style="heigh:50px; margin-left:50px;" for="edit" class=" control-label">Edit</label>
                                        <label><input name="edit[]" type="checkbox" value="{{$row->module_id}}" class="minimal" @if($row->edit_access == 1){{'checked'}} @endif></label>
                                        
                                        <label style="heigh:50px; margin-left:40px;" for="delete" class=" control-label">Delete</label>
                                        <label><input name="delete[]" type="checkbox" value="{{$row->module_id}}" class="minimal" @if($row->write_access == 1){{'checked'}} @endif></label>
                                    </div> 
                                @php
                                      $i++
                                @endphp
